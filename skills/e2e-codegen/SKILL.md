...
- **部分一致ロケータで別データを数えない**: `filter({ hasText })` や `getByText()` は**部分一致**なので、`<名前>-EDITED` のような**派生名を扱うシナリオでは前方一致の別行まで拾う**。件数 assert がそれで「実質同じものを数えているだけ」になり、意図と食い違ったまま green になる（実検証で発生。途中離脱＝`abandon` 観点で頻出）。派生名が絡む件数・存在の検証は**完全一致**で数える: `page.getByRole('listitem').filter({ has: page.getByRole('link', { name: title, exact: true }) })` のように、行内の意味要素のアクセシブルネームで厳密一致させる。
...
- **否定アサートの前に「その否定が意味を持つ状態」を positive assert で確定する**: `toHaveCount(0)` / `not.toHaveURL(...)` / 「エラーが出ないこと」といった**否定 assert は、画面が未ロードでも、障害注入が発火していなくても自明に通る**。否定を書くときは必ずその前段に、否定が意味を持つ前提を positive に確定する assert を置く——消滅検証なら「一覧のロード完了」を確定した**後段**で評価し、障害注入下の「遷移しない／通知が出ない」なら「**注入が実際に発火したこと**」（例: route ハンドラのカウンタを `await expect.poll(() => aborted).toBeGreaterThan(0)`）を先に検証する。既存の件数ガード（`toHaveCount(before+1)` を待ってから `nth`）も遷移 assert も、この原則の実例。**トリビアルな green は失敗より質が悪い**（気づけないため）。
...
- **明示保存ボタンが無い自動保存 UI では、「保存完了」と 1:1 対応する可視シグナルを実画面探索で確定してから、reload / 戻る / 離脱系の操作を書く**（時間待ちで代替しない）。**成功トーストが出ない実装は普通にある**——その場合は保存処理の**完了後にだけ**状態が変わる要素（undo・履歴・dirty 表示の解除など）を探して待機条件にする。永続の証明は入力欄の値ではなく、**リロード後の値か一覧への反映**で取る。
- **障害注入は「特定エンドポイント × メソッド」に絞って `page.route` で落とす**: `context.setOffline` や広域 glob は読み取りまで殺して画面ごと壊れ、何も検証できなくなる（glob が GET を拾うと発火カウンタも膨れる）。注入の発火は必ず positive 検証する（上記の否定アサート原則）。**クライアント SDK が失敗した書き込みを再送キューに積む実装では `unroute` だけでは足りない**——注入中の書き込みが解除後に流れて余分なデータが増えるため、解除後の正常系検証に入る前に**リロードで送信待ち状態を破棄する**（例: Firestore は Write チャネルだけ abort し、読み取り Listen は無傷にする）。
...
- **破壊的・自己完結シナリオの describe には `test.describe.configure({ mode: 'serial' })` を付ける。** `playwright.config.ts` は `fullyParallel: true` のため、同一データ空間に作用する作成/更新/削除テストを並列実行すると、別テストが作った/消した行と競合して flaky になる。serial で同 describe 内を直列化し、競合を避ける。**これは応急処置である。** 本来は各テストが**自分専用の隔離データ**（テストごとにユニークなアカウント/プロジェクト/seed）を setup で用意し teardown で消すことで、並列のまま安全にするのが筋。隔離が用意できない段階での暫定手段が serial だと理解しておく。
  - **`describe.serial` は同一ファイル内の競合しか防げない。** 複数のfeatureファイルが同一の外部データストア（DB/Firestore等）を共有して破壊的に編集する構成では、ファイル間・worker間の競合も起きる。**この場合のみ**、`playwright.config.ts` の `fullyParallel` を `false` に変更する（scaffoldの既定はこれを想定していない。隔離されたデータで書けるなら並列のままでよい）。この問題が実際に効くのは**ローカル実行時**（`workers` が未指定でCPU数に応じて並列する場合）に限られる——CIは既存の `workers: process.env.CI ? 1 : undefined` 分岐で既に1 workerのため、`fullyParallel` の値に関わらずファイル間の並行は起きない。したがって既存の `workers` のCI/ローカル分岐（ローカルは並列で速く、CIのみ直列にする設計）自体は変更しない。**この変更はcodegenが自動判断で行わない**——複数specファイルが同一の外部可変状態を共有するかはプロジェクト固有のアーキテクチャ判断であり、プロジェクト設定者（人間）が明示的に決める。
...
- **「消えたこと」の検証は必須。ただし検証を"ロード完了ゲートの後段"に置かないと、それ自体が自明に通る。** 後始末の最後に「作成名がもう存在しない」ことを検証する一文を必ず置く（無いと、teardown が実機で発火していなくても green のまま残骸が蓄積する。実検証で削除メニューが実 DOM で発火せず残骸が溜まった）。**だがこの対策 assert には二段目の穴がある**——一覧が非同期ロードなら、`if (await row.count())` ガードは未ロードの 0 を拾って**削除をスキップ**し、直後の `toHaveCount(0)` も**同じ未ロード状態で 0 を返して素通り**する。teardown が完全な no-op でも green になる（実検証で並列実行時に発生し、全 green のまま残骸が出た）。したがって:
  ...
  - **消滅検証のロケータは page 全域の `getByText(name)` ではなく、一覧行にスコープした完全一致で数える**（ダイアログ・トースト等の別出現や派生名を拾わない）。
...
- [ ] 複数のfeatureファイルが同一の外部データストア（DB/Firestore等）を破壊的に共有編集していないか（`describe.serial` はファイル内競合しか防げないため）。している場合、`playwright.config.ts` の `fullyParallel: false` への変更が必要である旨をユーザーに報告したか（codegenが自動で書き換えるのではなく、プロジェクト設定者の判断を仰ぐ。既存の `workers` のCI/ローカル分岐は変更不要）
...
- **(a') 期待の反証で収束** — 要素も動線も実在するが、**plan が期待した挙動が実装されていない**（通知が出ない・検証が無い・保存されるはずが破棄される等）と実画面観測で判明した場合。**推測で期待を緩めることは禁止**で、DOM 観測（＋可能ならプロダクトコードの裏取り）を根拠にしたうえで、次の4点セットで処理する:
  1. **spec は実挙動を確定アサートにする**——検証を消すのではなく「**起きないこと**」を積極的に検証する（例: 失敗通知が `toHaveCount(0)` であること＋否定アサート原則に従い注入発火を positive 検証）。
  2. plan の該当シナリオへ「**確定（Step3 実走）**」として実挙動を書き戻す。
  3. plan の「未確認・要レビュー」に**プロダクト不具合候補として「要起票」を明記**する（起票自体はしない。妥当性は承認ゲート②で人間が判断する）。
  4. spec に「**修正が入ったら期待値を反転する**」旨と、反転後の assert をコメントで残す。
  これは (b) の残差ではない（テストは green で確定する）。**この出口を「実挙動に合わせて green にする」抜け道に使わない**——観測の根拠と要起票の記録が無いなら使ってはいけない。
...
### plan への書き戻し（plan は生き文書・ただし「事実の確定」に限る）

収束ループで確定した事実は、spec のコメントだけに持たせず **plan へ書き戻す**。plan が正本であり（`/e2e-audit` は plan を読む）、次回以降の feature・別の作業者はコメントではなく plan を読むため。**1本目の feature で spec コメントに留めた教訓が、2本目で伝播せず同型の穴として再発した実例がある。**

- **書き戻す対象＝事実の確定だけ**: 未確定だったオラクル HOW・保存完了の待機条件・二択に割れていた実挙動・シード方法・plan 側の誤り（遷移マップ参照ミス等）。「**確定（Step3 実走）**」のような出所ラベルを付け、何がどう確定したかを追記する（元の記述を黙って書き換えず、経緯を残す）。
- **書き戻さない対象**: シナリオの追加・削除、`status` の変更、除外範囲の変更。これらは**承認ゲートを経る**ものであり、収束ループが独断で動かすと Step2 の合意が骨抜きになる。必要なら差分を報告してユーザー判断を仰ぐ。
- **書き戻しの実行主体はオーケストレータ／codegen 本体**。収束ループのサブエージェントは**差分の報告に留める**（同一ファイルへの並列書き込みを避けるため）。
