...
- [critical] は **実検証で実際に壊れた外部グラウンドな失敗モード**、または当該スキルの定義責務にアンカーする。
...
| # | 項目 | 種別 |
|---|------|------|
| CE1 | **新規作成行を「出現待ちなしの nth()/last()/first()」で掴んでいない**。新規行は keyboard 直接入力か `toHaveCount(before+1)` で件数増加を待ってから特定 | **critical**（実検証で `.last()` が既存行を改名した破壊事故） |
